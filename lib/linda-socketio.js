    ...
    Linda.prototype.listen = function(opts) {
      ...
      this.io.sockets.on('connection', function(socket) {
        ...
        socket.on('__linda_take', function(data) {
          _this.tuplespace(data.tuplespace).take(data.tuple, function(err, tuple) {
            return socket.emit("__linda_take_" + data.id, tuple);
          });
          return _this.emit('take', data);
        });
        socket.on('__linda_read', function(data) {
          _this.tuplespace(data.tuplespace).read(data.tuple, function(err, tuple) {
            return socket.emit("__linda_read_" + data.id, tuple);
          });
          return _this.emit('read', data);
        });
        return socket.on('__linda_watch', function(data) {
          _this.tuplespace(data.tuplespace).watch(data.tuple, function(err, tuple) {
            return socket.emit("__linda_watch_" + data.id, tuple);
          });
          return _this.emit('watch', data);
        });
      });
      return this;
    };
